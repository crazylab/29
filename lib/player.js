...
Player.prototype = {
	...
	getCardIDOf : function(suit){									//Modify to fix encapsulation
	...
	throwACard : function(){									//Modify to fix encapsulation
		return _.first(this.hand.splice(0,1))
	},

	throwAValidCard : function(playedCards){				//Modify to fix encapsulation
		if(playedCards.isStartOfRound())
			return this.throwACard();

		var runningSuit = playedCards.getRunningSuit();
		var throwableCardID = this.getCardIDOf(runningSuit);

		return throwableCardID ? this.throwCard(throwableCardID) : this.throwACard();
	}
};
