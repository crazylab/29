...
var PlayedCards = require('./playedCards');
...
var beforeTrumpShow = function(comparingCard,startingSuit,standCard){
	if (startingSuit[0] == comparingCard.getCardSuit()[0]) {
...
var afterTrumpShow = function(comparingCard,startingSuit,standCard,trumpSuitID){
	if (comparingCard.getCardSuit()[0] == trumpSuitID[0] && !standCard.isTrumpShown())
		standCard = comparingCard;
	else if(comparingCard.getCardSuit()[0] == trumpSuitID[0] && standCard.isTrumpShown()){
		if(standCard.getCardSuit()[0] != trumpSuitID[0])
			standCard = comparingCard;
		if(standCard.getCard().rank > comparingCard.getCard().rank && standCard.getCardSuit()[0] == trumpSuitID[0])
			standCard = comparingCard;
	}
	else if(comparingCard.getCardSuit()[0] != trumpSuitID[0] && standCard.getCardSuit()[0] != trumpSuitID[0])
...
Game.prototype = {
	...
	pairChecking : function () {
		var trumpSuitID = this.trump.suit;
		this.team_1.players.forEach(function(player){
			player.checkPair(trumpSuitID);
		});
	...
	setRoundSequence : function(roundWinner){
		if(roundWinner == undefined)
			_.first(this.roundSequence).setTurn();
		...
		var roundStartingPlayer = _.first(this.roundSequence);
		roundStartingPlayer.setTurn();
		this.startAutoPlayTimerToPlayCard(roundStartingPlayer);
	},
	...
	storeWonCards : function(winner){
		...
		this[roundWinningTeam].wonCards = this[roundWinningTeam].wonCards.concat(this.playedCards.getAllPlayedCardStatus());
	},

	startNewRound : function(){
		var winnerID = this.roundWinner();

		_.last(this.roundSequence).revokeTurn();
		this.setRoundSequence(winnerID);
		var game = this;
		this.storeWonCards(winnerID);

		setTimeout(function(){
			game.playedCards = new PlayedCards();
		},1500);
	},
	...
	handleCompletionOfRound : function(){
		...
		if (this.isGameFinished()) {
			this.autoPlayTimer.close();
			this.updateScore();
			this.resetGameVariables();
		}
	},

	startAutoPlayTimerToPlayCard : function(currentPlayer){
		var playerID = currentPlayer.getID();
		var throwableCardID = currentPlayer.getAThrowableCardID(this.playedCards);

		var self = this;
		this.autoPlayTimer = setTimeout(function(){
			self.playCard(playerID, throwableCardID);
		}, 45000);
	},
	...
	rotateTurn : function(){
		...
		this.roundSequence[previousPlayerIndex].revokeTurn();
		var currentPlayer = this.roundSequence[previousPlayerIndex+1];

		currentPlayer.setTurn();
		this.startAutoPlayTimerToPlayCard(currentPlayer);
	},

	nextTurn : function(){
		this.updateActivityTime();
		if(this.playedCards.isEndOfRound())
	...
	isJustNowTrumpRevel : function(){
		return !this.playedCards.isStartOfRound() && !this.playedCards.isTrumpOpen() && this.trump.open==true;
	},
	...
	isValidCardToThrow : function(cardID, player){
		if(this.playedCards.isStartOfRound()) return true;
		var runningSuitID = this.playedCards.getRunningSuit()[0];
		var requestedSuitID = cardID[0];
		var hasRunningSuit = player.hasSameSuitCard(runningSuitID);
		if(this.isJustNowTrumpRevel())
			return this.isValidCardToPlayAfterAskForTrumpSuit(player,requestedSuitID);

		var isCardPresent = player.hasCard(cardID);
		return isCardPresent && hasRunningSuit && (runningSuitID == requestedSuitID)	//read carefully
	...
	ableToAskForTrumpSuit : function(playerHand){
		if(this.playedCards.isStartOfRound())
			return false;
		var runningSuit = this.playedCards.getRunningSuit();
	...
	roundWinner : function (){
		var playedCards = this.playedCards.getAllPlayedCardStatus();
		var trumpSuitID = this.trump.suit;
		var startingSuit = this.playedCards.getRunningSuit();
		var standCard = playedCards[0];
		for (var i = 1;i < playedCards.length;i++){
			if (!playedCards[i].isTrumpShown() || startingSuit[0] == trumpSuitID[0])
				standCard = beforeTrumpShow(playedCards[i],startingSuit,standCard)
			else
				standCard = afterTrumpShow(playedCards[i],startingSuit,standCard,trumpSuitID);
		};
		var winner = this.getPlayer(standCard.playerID);
		winner.addPoint(calculatePoint(this.playedCards.getAllCards()));
	...
	getPlayedCards : function(playerID){
		var self = this;
		var playedCards = this.playedCards.getAllCardsWithPlayerID();
		return playedCards.map(function(playedCard){
			var result = {
				card : playedCard.card,
				relation : self.whoAreYou(playedCard.playerID, playerID)
			};
			return result;
		});
	},
	...
	playCard : function(playerID, cardID){
		...
		if(player.turn && this.isValidCardToThrow(cardID , player)){
			...
			this.playedCards.pushCard(player.id, thrownCard, this.trump.open);
			this.autoPlayTimer.close();
			this.nextTurn();
			return true;
		}
