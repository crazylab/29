...
var getCardOfJack = function(card){
	return card.name == 'J';
}
...
Game.prototype = {
	...
	isValidCardToThrow : function(cardID, player){
		...
		var startingCard = this.playedCards[0].card.id;
	...
	hasFourJackInAHand : function(){
		for(var playerIndex=0; playerIndex<this.distributionSequence.length; playerIndex++){
			var cardsOfJack = this.distributionSequence[playerIndex].hand.filter(getCardOfJack);
	...
	initializeGameAfterCancelation : function(){
		this.resetBidAndTrump();
		console.log(this.team_1.players[0]);
	...
	playCard : function(playerID, cardID){
		...
		if(player.turn && this.isValidCardToThrow(cardID , player.hand)){
			var thrownCard = player.throwCard(cardID);
			this.playedCards.push({player:player.id,card:thrownCard,trumpShown:this.trump.open});
			this.nextTurn();
			return true;
		}
		return false;
