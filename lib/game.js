...
Game.prototype = {
 ...
 //    },
	getRelationship : function(playerID){
		...
		relation.opponent_1 = _.first(relation.opponentTeam.players);
		relation.opponent_2 = _.last(relation.opponentTeam.players);
		return relation;
	},

	setPermissionForSeeHandCards : function(relationship){
		var status = {};
	...
	getStatus : function(playerID){
		...
		var status = this.setPermissionForSeeHandCards(relationship);

		status.bid = this.getFinalBidStatus();
		var topBidder = this.bid.player == playerID;
		status.isBidWinner = topBidder && !this.trump.suit;
	...
	setAllPlayersTurnFalse : function(){
		this.team_1.players[0].turn = false;
		this.team_1.players[1].turn = false;
		this.team_2.players[0].turn = false;
		this.team_2.players[1].turn = false;
	...
	setBid : function(playerID, value){
		if(value == 'Pass' && this.getBid().value == 0){
			this.bid.player = playerID;
			this.bid.value = 16;
		}
		else if(value < 16 || value <= this.bid.value || value > 28)
			return;
		else if(value != 'Pass'){
			this.bid.player = playerID;
			this.bid.value = value;
